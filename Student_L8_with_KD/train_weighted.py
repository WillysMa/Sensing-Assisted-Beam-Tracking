...
- Use --dynamic_time_weight to enable dynamic time weight adjustment based on KL divergence from training data
- Without this flag, time weights remain fixed throughout training
- Time weights are updated at intervals that start large and decrease over time to encourage more frequent updates in later training stages
- KL divergence is calculated from training data (not validation data) to determine optimal time weights
...
def parse_args():
    ...
    parser.add_argument('--lr', type=float, default=8.5e-4, help='Learning rate')
    ...
    parser.add_argument('--alpha', type=float, default=0.3, help='Weight for distillation loss (1-alpha for task loss)')
    ...
    parser.add_argument('--kappa', type=float, default=5.0, help='kappa>1: focus on large KLs, kappa=0:uniform weighting')
    parser.add_argument('--penalty_weight', type=float, default=10.0, help='Penalty weight for KL divergence')
    ...
    parser.add_argument('--gru_num_layers_student', type=int, default=2, help='Number of GRU layers for student')
    ...
    parser.add_argument('--attention_mode', type=str, default=2, choices=[1,2,3], help='Attention mode 2 perform best')
    parser.add_argument('--num_heads', type=int, default=8, help='Number of heads for multi-head attention')
    parser.add_argument('--use_cbam', action='store_true', default=False, help='Use CBAM make worse performance')
...
class DistillationLoss(nn.Module):
    ...
    def forward(self, student_logits, teacher_logits, targets, 
        ...
        if self.args.kd_mode == 0:
            ...
            kl_loss_batch_mean = kl_loss_reshaped.mean(dim=0) # [seq_len]
            ...
            # distillation_loss = distillation_loss * (self.args.temperature ** 2) + self.args.penalty_weight * torch.max(kl_loss_batch_mean)
            distillation_loss = distillation_loss * (self.args.temperature ** 2) 
            # distillation_loss = torch.max(kl_loss_batch_mean)* (self.args.temperature ** 2) 
...
def train_model(teacher_model, student_model, dataloaders, args, optimizer, scheduler, device, save_path, input_linear_mapping=None):
    ...
    if args.kd_mode == 0:
        ...
        if args.kd_mode == 1:
            ...
            if args.dynamic_time_weight:
                ...
                print(f"=====Weight adjustment: Interval-based (starts large, decreases over time)=====")
            else:
    ...
    best_test_loss = 1e+3
    # For dynamic time weight adjustment - track last weight update epoch
    last_weight_update_epoch = -1
    if args.resume:
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # Initialize KL divergence tracking for this epoch (for training data)
        train_kl_div_per_position = torch.zeros(args.num_pred + 1, device=device)
        train_num_samples = 0
        ...
        with tqdm(dataloaders['train'], unit="batch", file=sys.stdout) as tepoch:
            for i, (img, beam, label) in enumerate(tepoch, 0):
                ...
                # Calculate KL divergence per position for training data (for weight updates)
                if args.kd_mode == 1:
                    with torch.no_grad():
                        student_soft = F.log_softmax(student_outputs / args.temperature, dim=-1)
                        ...
                        train_kl_div_per_position += kl_div_per_sample.sum(dim=0)  # [seq] - sum over batch
                        train_num_samples += student_outputs.size(0)
        ...
        # Calculate average training KL divergence per position for this epoch
        avg_train_kl_div_per_position = None
        if args.kd_mode == 1 and train_num_samples > 0:
            avg_train_kl_div_per_position = train_kl_div_per_position / train_num_samples
            s_values = avg_train_kl_div_per_position.cpu().numpy()
            div_history.append(s_values)
            print(f'Average training KL divergence per position: {avg_train_kl_div_per_position.cpu().numpy()}', flush=True)
        
        # Validation - no need to pass teacher model since we're using training KL divergence
        ...
        # Dynamic time weight adjustment logic (interval-based) - using training KL divergence
        if args.dynamic_time_weight and args.kd_mode == 1 and avg_train_kl_div_per_position is not None:

            # Apply softmax transformation with temperature parameter kappa
            
            # For numerical stability, subtract the maximum value before exponentiating
            s_values_stable = s_values - np.max(s_values)
            exp_values = np.exp(args.kappa * s_values_stable)
            new_weights = exp_values / np.sum(exp_values) * (args.num_pred + 1)
            
            time_weights_history.append(new_weights)

            # Calculate current interval for weight updates
            current_interval = calculate_weight_update_interval(epoch)
            
            # Check if it's time to update weights
            if epoch - last_weight_update_epoch >= current_interval:              
                old_weights = args.time_weights.copy()
                args.time_weights = new_weights.tolist()
                
                print(f"\n=====ADJUSTING TIME WEIGHTS (INTERVAL-BASED, TRAINING DATA)=====")
                print(f"Epoch {epoch}: Time for weight update (interval: {current_interval})")
                print(f"Old weights: {old_weights}")
                print(f"New weights: {args.time_weights}")
                print(f"Training KL divergence per position: {s_values}")
                print("=====WEIGHT ADJUSTMENT COMPLETE=====\n")
                
                # Update last weight update epoch
                last_weight_update_epoch = epoch
                
                # Save weight adjustment info to file
                with open(os.path.join(save_path, 'weight_adjustments.txt'), "a") as f:
                    f.write(f"\nEpoch {epoch}: Interval-based Weight Adjustment (Training Data)\n")
                    f.write(f"Update interval: {current_interval} epochs\n")
                    f.write(f"Old weights: {old_weights}\n")
                    f.write(f"New weights: {args.time_weights}\n")
                    f.write(f"Training KL divergence per position: {s_values}\n")
                    f.write("-" * 50 + "\n")
...
def main():
    ...
    train_loader = DataLoader(
        ...
        batch_size=args.train_batch_size, shuffle=True, generator=generator, num_workers=args.num_workers
    )
    test_loader = DataLoader(
        DataFeed(data_root, test_dir, args.seq_length_teacher, transform=proc_pipe, portion=args.dataset_pct),
        batch_size=args.test_batch_size, shuffle=False, generator=generator, num_workers=args.num_workers
    )
    ...
    teacher_model = ImageModalityNet_MHA(args.feature_size, args.num_classes, GRU_PARAMS_teacher, num_heads=args.num_heads, 
                                 use_cbam=args.use_cbam, attention_mode=args.attention_mode)
    ...
    student_model = ImageModalityNet_MHA(args.feature_size, args.num_classes, GRU_PARAMS_student, num_heads=args.num_heads, 
                                 use_cbam=args.use_cbam, attention_mode=args.attention_mode)
