...
class CAM(nn.Module):
    def __init__(self, channels, r):
        ...
        self.r = r
        # Ensure intermediate layer has at least 1 channel to avoid division by zero
        intermediate_channels = max(1, self.channels // self.r)
        self.linear = nn.Sequential(
            nn.Linear(in_features=self.channels, out_features=intermediate_channels, bias=True),
            nn.ReLU(inplace=True),
            nn.Linear(in_features=intermediate_channels, out_features=self.channels, bias=True))
...
class ImageFeatureExtractor_CBAM(nn.Module):
    def __init__(self, n_feature, in_channel=1, reduction_ratio=4):
        super(ImageFeatureExtractor_CBAM, self).__init__()
        ...
        # CBAM after first conv block - use smaller reduction ratio for small channels
        self.cbam1 = CBAM(channels=4, r=1)
        ...
        # CBAM after second conv block
        self.cbam2 = CBAM(channels=8, r=2)
        ...
        # CBAM after third conv block
        self.cbam3 = CBAM(channels=16, r=reduction_ratio)
        
        # Fourth conventional layer
        ...
        # CBAM after fourth conv block
        self.cbam4 = CBAM(channels=32, r=reduction_ratio)
        
        # Fifth conventional layer
        ...
        # Final CBAM
        self.cbam5 = CBAM(channels=64, r=reduction_ratio)
        
        # Flatten layer
        self.flatten = nn.Flatten()
        
        # Fully connected layers for feature dimensionality reduction
        self.fc_layer = nn.Sequential(
             nn.Linear(64 * 7 * 7, 512),
             nn.ReLU(),
             nn.Dropout(0.5),
    ...
    def forward(self, x):
        ...
        for t in range(seq_length):
            frame = x[:, t, :, :, :]  # Get frame at time step t
            
            # Apply conv layers with CBAM attention modules
            frame_features = self.conv1(frame)
            frame_features = self.cbam1(frame_features)  # Apply CBAM after conv1
            
            frame_features = self.conv2(frame_features)
            frame_features = self.cbam2(frame_features)  # Apply CBAM after conv2
            
            frame_features = self.conv3(frame_features)
            frame_features = self.cbam3(frame_features)  # Apply CBAM after conv3
            
            frame_features = self.conv4(frame_features)
            frame_features = self.cbam4(frame_features)  # Apply CBAM after conv4
            
            frame_features = self.conv5(frame_features)
            frame_features = self.cbam5(frame_features)  # Apply CBAM after conv5
        ...
        spatial_features = torch.stack(spatial_features, dim=1)  # Shape: (batch_size, seq_length, n_feature)
        return spatial_features


class ImageModalityNet_MHA(nn.Module):
    def __init__(self, feature_size, num_classes, gru_params, num_heads=8, **kwargs):
        ...
        self.use_cbam = kwargs.get('use_cbam')
        
        if self.use_cbam:
            self.feature_extraction = ImageFeatureExtractor_CBAM(feature_size) # image input only
        else:
        ...
        self.attention_mode = kwargs.get('attention_mode')
        # self.attenion_mode = 3
        if self.attention_mode == 3:
            # Context vector generation
            self.context_projection = nn.Linear(gru_hidden_size*2, gru_hidden_size)
    ...
    def forward(self, image_batch, beam):
        ...
        if self.attention_mode == 1:
            enhanced_seq_out = attn_output            
        elif self.attention_mode == 2:
            enhanced_seq_out = attn_output + Seq_out
        elif self.attention_mode == 3:
            enhanced_seq_out = self.context_projection(torch.cat((attn_output,Seq_out), dim=2))
...
class ImageModalityNet(nn.Module):
    ...
    def forward(self, image_batch, beam):
        ...
        if self.Temporal_attention:
            attn_weights = self.temporal_attention(Seq_out) # (batch_size, seq_len, 1)
            ...
            context_vector = torch.sum(Seq_out * attn_weights, dim=1) # (batch_size, gru_hidden_size)
...
if __name__ == "__main__":
    ...
    model = ImageModalityNet_MHA(FEATURE_SIZE, NUM_CLASS, GRU_PARAMS, num_heads=8)
    print(summary(model, image_input, beam[:,0], show_input=True, show_hierarchical=True))
    ...
    print("\n" + "="*50)
    print("Testing CBAM-enhanced model:")
    print("="*50)
    
    # Test the model with CBAM enabled
    model_cbam = ImageModalityNet_MHA(FEATURE_SIZE, NUM_CLASS, GRU_PARAMS, num_heads=8, use_cbam=True)
    print(summary(model_cbam, image_input, beam[:,0], show_input=True, show_hierarchical=True))
    
    # Summarize and print all trainable parameters for CBAM model
    trainable_params_cbam = sum(p.numel() for p in model_cbam.parameters() if p.requires_grad)
    print(f"\nTotal number of trainable parameters (with CBAM): {trainable_params_cbam}")
    
    # Test forward pass
    output_cbam = model_cbam(image_input, beam[:,0])
    print(f"\nCBAM Model Output Shape: {output_cbam[0].shape}")
    print(f"CBAM Model Features Shape: {output_cbam[1].shape}")
    print(f"CBAM Model Enhanced Seq Output Shape: {output_cbam[2].shape}")
    ...
    output = model(image_input, beam[:,0])
    print(output[0].shape)  # 应该输出torch.Size([8, 10]) 如果是10分类问题的话